...
let timeZoneList = world_timezones.sort();

for (let i = 0; i < timeZoneList.length; i++){
    let optn = timeZoneList[i];
...
select.addEventListener("change", function(){
    ...
    nd = new Date();
    let isTime = nd.toLocaleString('en-GB',{timeZone: isSelected, timeZoneName:'short'});
    showTime(isTime, isSelected);
    console.log(isTime);
});
...
function showTime(time, myZone) {
    ...
    // Offset = 1;
    // stockholm = utc + (3600000*Offset);

    
    
    // let sliceHours = nd.getHours();
    // let sliceMinutes = nd.getMinutes();
    // let sliceSeconds = nd.getSeconds();


    // let hours = ("0" + sliceHours).slice(-2);
    // let minutes = ("0" + sliceMinutes).slice(-2);
